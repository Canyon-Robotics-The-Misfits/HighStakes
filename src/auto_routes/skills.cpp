...
AUTO_ROUTE(auto_routes::skills_start_segment) {
    ...
    drive_controller->drive_to(pose(76.8438 + 2 -0.5 -1, 13.1988 + 1.5 + 2, -160.197_deg), { r: 10, min_speed: 30 });
    pros::delay(50);
...
AUTO_ROUTE(auto_routes::skills)
{
    ...
    auto drive_to_ring_stack_1 = drive_controller->boomerang(pose(12 -2, 144 - 24 - 5, 0_deg), { min_speed: 30, async: true });
    pros::delay(800);
    intake->set_state(IntakeState::WALL_STAKE);
    ...
    drive_controller->boomerang(pos(24 +1, 12 - 1), { min_speed: 30 });
    pros::delay(50);
    drive_controller->face_angle(-45_deg, { threshold: 10_deg, chained: true });
    ...
    drive_controller->boomerang(pos(72 + 24, 72 + 24), { threshold: 9, min_speed: 60 });
    auto face_third_goal = drive_controller->face_angle(135_deg, { threshold: 10_deg, chained: true, async: true });
    pros::delay(200);
    intake->set_state(IntakeState::DISABLED);
    face_third_goal->await();
    drive_controller->boomerang(pose(72, 144 - 24 +1, 105_deg), { backwards: true, lead: 0.4, threshold: 8, min_speed: 60 });
    drive_controller->drive_time(-60, 50);
    clamp.extend();
    drive_controller->drive_time(-60, 50);
    ...
    drive_controller->boomerang(pose(72 + 10, 144 - 30, 90_deg), { backwards: true });
    clamp.retract();
    pros::delay(100);
    ...
    drive_controller->drive_to(pose(72 -1, 144 - 15, 0_deg), { threshold: 1.5, min_speed: 30 });
    pros::delay(50);
    arm->set_state(ArmState::LOAD);
    pros::delay(350);
    drive_controller->drive(-4, { min_speed: 60, chained: true });
