...
AUTO_ROUTE(auto_routes::skills)
{
    ...
    drive_controller->face_angle(-15_deg, { min_speed: 40, chained: true });
    pros::delay(200);
    ...
    rm->stop_intake();
    face_goal_two->await();
    drive_controller->boomerang(pose(144 - 48, 144 - 12 +3, -30_deg + 180_deg), { backwards: true, threshold: 12, angle_priority_threshold: 17, max_speed: 60, min_speed: 63.5 });
    pros::delay(100);
    clamp.extend();
    ...
    rm->load();
    drive_controller->boomerang(pos(72, 144 - 24), { backwards: true, threshold: 3, min_speed: 40 });
    clamp.extend();
    pros::delay(50);

    drive_controller->drive_to(pose(72 +3, 144 - 15, 0_deg), { r: 2, min_speed: 40 });
    pros::delay(100);
    mechanism::distance_reset(odometry, true, true, false);
    rm->set_lb_override(true);
    ...
    drive_controller->drive(-1, { min_speed: 25, chained: true });
    lb_lift_push->extend();
