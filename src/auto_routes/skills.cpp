...
AUTO_ROUTE(auto_routes::skills)
{
    ...
    pros::delay(100);
    rm->load();
    drive_controller->boomerang(pose(144 - 48, 48, 15_deg), { threshold: 5, min_speed: 40 });
    ...
    rm->set_lb_override(false);
    auto turn_after_wall_stake_one = drive_controller->face_angle(180_deg, { min_speed: 60, chained: true, async: true });
    pros::delay(150);
    rm->intake_reverse();
    turn_after_wall_stake_one->await();
    rm->intake();
    drive_controller->boomerang(pose(144 - 24 +1.5, 12, 180_deg), { threshold: 2, angle_priority_threshold: 10,  max_speed: 90 });
    pros::delay(600);
    ...
    // get ring + goal
    auto leave_corner_one = drive_controller->boomerang(pos(144 - 24 +4, 144 - 48), { threshold: 6, min_speed: 20, async: true });
    pros::delay(300);
    rm->intake();
    ...
    drive_controller->boomerang(pose(24, 48 +7, 185_deg), { lead: 0.4, threshold: 6, min_speed: 80 });
    drive_controller->boomerang(pose(24, 12 +4, 180_deg), { threshold: 3, angle_priority_threshold: 6,  max_speed: 90 });
    pros::delay(600);
