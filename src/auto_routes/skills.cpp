...
AUTO_ROUTE(auto_routes::skills)
{
    ...
    auto turn_after_wall_stake_one = drive_controller->face_angle(180_deg, { threshold: 10_deg, min_speed: 60, chained: true, async: true });
    pros::delay(250);
    // rm->intake_reverse();
    turn_after_wall_stake_one->await();
    // rm->intake();
    drive_controller->boomerang(pose(144 - 24 +1.5, 24 +2, 180_deg), { threshold: 2, angle_priority_threshold: 10,  max_speed: 90, min_speed: 30 });
    pros::delay(100);
    mechanism::distance_reset(odometry, false, true, false);
    drive_controller->drive(10, { max_speed: 80, min_speed: 40, chained: true });
    ...
    drive_controller->boomerang(pose(144 - 48, 144 - 12 +1, -30_deg + 180_deg), { backwards: true, threshold: 7, angle_priority_threshold: 15, max_speed: 60, min_speed: 40 });
    pros::delay(100);
    clamp.extend();
    ...
    rm->intake();
    pros::delay(300);
    drive_controller->boomerang(pos(144 - 24, 144 - 24 -2), { threshold: 7, min_speed: 30 });
    ...
    drive_controller->drive_to(pose(72 +4 -0.5, 144 - 24 -4 -0.5, -30_deg), { r: 8, backwards: true, threshold: 1, max_speed: 80, min_speed: 40 });
    drive_controller->drive(-2, { max_speed: 40, min_speed: 40, chained: true });
    clamp.extend();
    pros::delay(50);

    drive_controller->drive_to(pose(72 +1, 144 - 15, 0_deg), { r: 1, min_speed: 60 });
    // pros::delay(50);
    // mechanism::distance_reset(odometry, true, true, false);
    rm->set_lb_override(true);
    ...
    drive_controller->drive(-2, { threshold: 0, min_speed: 25, chained: true });
    lb_lift_push->extend();
    lb->move(127);
    pros::delay(400);
    drive_controller->drive(-4, { min_speed: 80, chained: true });
    ...
    lb_lift_push->retract();
    drive_controller->boomerang(pose(24 +2, 48 +3, 190_deg), { lead: 0.5, threshold: 6, min_speed: 80 });
    drive_controller->boomerang(pose(24, 24 +4, 180_deg), { threshold: 3, angle_priority_threshold: 6,  max_speed: 90 });
    pros::delay(100);
    ...
    rm->set_lb_override(false);
    // rm->intake_reverse();
    drive_controller->face_angle(0_deg, { threshold: 10_deg, min_speed: 60, chained: true });
    rm->intake();
    ...
    drive_controller->boomerang(pose(48, 144 - 12 -1, 30_deg + 180_deg), { backwards: true, threshold: 9, angle_priority_threshold: 15, max_speed: 60, min_speed: 40 });
    pros::delay(100);
    clamp.extend();
    ...
    clamp.retract();
    drive_controller->drive(-6, { timeout: 500, max_speed: 100, min_speed: 60, chained: true });
    rm->intake_reverse();
    drive_controller->drive_time(80, 200);
