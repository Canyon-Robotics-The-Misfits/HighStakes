...
AUTO_ROUTE(auto_routes::positive_red)
{
    ...
    pros::delay(50);
    intake->set_state(IntakeState::HOOD);
    drive_controller->drive_to(pose(72 + 17, 29, -125_deg), { r: 8, threshold: 2, min_speed: 35 });
    ...
    intake->set_state(IntakeState::HOOD);
    auto drive_to_alliance_stake = drive_controller->drive_to(pose(72+8 -1.5, 14, -148_deg), { threshold: 1, timeout: 1000, min_speed: 30, async: true });
    ...
    // get corner
    arm->set_state(ArmState::ALLIANCE_STAKE);
    auto drive_to_corner = drive_controller->boomerang(pose(144 - 18, 18, 65_deg), { lead: 0.4, threshold: 8, angle_priority_threshold: 10, min_speed: 60, async: true });
    intake->set_state(IntakeState::HOOD);
    pros::delay(1000);
    intake->set_state(IntakeState::DISABLED);
    ...
    intake->set_state(IntakeState::HOOD);
    drive_controller->drive_time(70, 400);
    pros::delay(100);
    auto leave_corner = drive_controller->drive(-18, { threshold: 1, min_speed: 60, chained: true, async: true });
    pros::delay(300);
    intake->set_state(IntakeState::DISABLED);
    leave_corner->await();
    ...
    drive_controller->face_point(lib15442c::Vec(144 - 18, 72 - 16), 180_deg, { threshold: 5_deg, chained: true });
    ...
    intake->set_state(IntakeState::HOOD);
    drive_controller->boomerang(pos(144 - 48, 48 +4), { threshold: 5 , min_speed: 6 });
