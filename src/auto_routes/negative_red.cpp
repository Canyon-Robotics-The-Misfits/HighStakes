...
AUTO_ROUTE_PARAM(auto_routes::negative_red, bool elims)
{
    ...
    auto drive_to_goal = drive_controller->boomerang(pos(48, 48 +1), { backwards: true, threshold: 3, min_speed: 60, async: true });
    pros::delay(350);
    ...
    drive_controller->boomerang(pos(15 -2, 15 -2), { threshold: 2, min_speed: 40 });

    drive_controller->face_point(lib15442c::Vec(4, 0 +4), 5_deg, { threshold: 3_deg });
    pros::delay(50);
    intake->set_state(IntakeState::DISABLED);
    drive_controller->drive_time(50, 700);
    intake->set_state(IntakeState::HOOD);
    pros::delay(150);
    drive_controller->drive(-3.5, { min_speed: 40, chained: true });
    pros::delay(50);
    intake_lift.extend();
    pros::delay(100);
    ...
    arm->set_state(ArmState::ALLIANCE_STAKE);
    intake->set_state(IntakeState::HOOD);
    auto drive_to_alliance_stake = drive_controller->drive_to(pose(72-8 +1.5 - 2 -1, 14 +0.5 -1, 148_deg -3_deg), { r: 8, threshold: 1, timeout: 2000, min_speed: 25, async: true });
    ...
    if (!elims)
    {
        ...
        drive_controller->boomerang(pos(47, 47), { backwards: true, threshold: 7, min_speed: 60 });
