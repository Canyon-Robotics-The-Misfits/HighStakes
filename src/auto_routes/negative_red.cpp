...
AUTO_ROUTE(auto_routes::negative_red)
{
    ...
    drive_controller->face_point(lib15442c::Vec(72, 0), 10_deg, { chained: true });
    auto drive_to_alliance_stake = drive_controller->drive_to(pose(72-8, 12, 145_deg), { r: 4, threshold: 1, timeout: 1000, min_speed: 30, async: true });
    ...
    // get line rings
    drive_controller->drive_to(pose(25, 72 - 12 + 3, -90_deg), { max_speed: 110, min_speed: 40 });
    pros::delay(100);
    ...
    // get corner 
    arm->set_state(ArmState::ALLIANCE_STAKE);
    drive_controller->boomerang(pos(16, 16), { threshold: 2 });
