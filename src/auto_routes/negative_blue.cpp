...
AUTO_ROUTE(auto_routes::negative_blue)
{
    ...
    drive_controller->drive(-20, { min_speed: 40, chained: true });
    ...
    drive_controller->boomerang(pos(144 - 24, 48), { threshold: 10 });
    pros::delay(600);
    drive_controller->drive(4, { min_speed: 80, chained: true });

    // wall stake
    auto drive_to_wall_stake = drive_controller->drive_to(pose(144 - 11.5, 72 - 8, 45_deg), { r: 4, timeout: 1500, min_speed: 40, async: true });
    pros::delay(600);
    intake->set_state(IntakeState::DISABLED);
    intake_lift.extend();
