...
AUTO_ROUTE(auto_routes::negative_blue)
{
    ...
    // score wall stake
    // wall stake
    auto drive_to_wall_stake = drive_controller->drive_to(pose(144 - 11.5, 72 - 8, 50_deg), { r: 4, timeout: 1500, min_speed: 40, async: true });
    pros::delay(400);
    intake->set_state(IntakeState::HOOD);
    ...
    drive_controller->boomerang(pos(144 - 17, 17), { threshold: 2, min_speed: 30 });
    ...
    // get middle ring
    auto drive_to_middle_ring = drive_controller->drive_to(pose(72 + 9, 24, -90_deg), { min_speed: 40, async: true });
    pros::delay(900);
    arm->set_state(ArmState::ALLIANCE_STAKE);
    intake->set_state(IntakeState::HOOD);
    pros::delay(100);
    ...
    drive_controller->drive_to(pose(72 + 15, 72 - 15, -45_deg), { r: 6, min_speed: 40 });
}